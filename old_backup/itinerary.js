/* ============================================================
   Itinerary / Trip Planner - לוח זמנים לטיול
   Calendar-based trip planner with Gantt view, links, GPX segments
   ============================================================ */

// ============= Constants =============
const ITINERARIES_KEY = 'mytravel-itineraries';

// ============= State =============
let itineraries = [];       // All saved trips
let activeItineraryId = null;  // Currently viewed trip
let editingDayDate = null;     // Date being edited in modal
let currentItineraryView = localStorage.getItem('mytravel-itinerary-view') || 'timeline'; // 'timeline', 'compact', 'calendar'
let jewishHolidaysMap = {};
    ...
    if (window.IS_FIREBASE_CONFIGURED && window.db) {
        window.db.collection('itineraries').onSnapshot(snapshot => {
            ...
            if (savedActiveId && itineraries.some(it => it.id === savedActiveId)) {
                activeItineraryId = savedActiveId;
            }
...
function syncItineraryToFirebase(itinerary) {
...
function deleteItineraryFromFirebase(id) {
    if (window.IS_FIREBASE_CONFIGURED && window.db) {
...
// ============= CRUD Operations =============
function createItinerary(name, startDate, endDate, color) {
    const id = 'itin-' + Date.now();
    const start = new Date(startDate);
    const end = new Date(endDate);
    const days = [];

    // Generate a day entry for each date in range
...
function updateDay(itineraryId, date, updates) {
...
function getDayNumber(itinerary, dateStr) {
...
function renderGanttView(itineraryId) {
    ...
    // Load Jewish Holidays for this year and next year asynchronously
    const start = new Date(itin.startDate + 'T00:00:00');
    ...
    const daysHtml = itin.days.map((day, idx) => {
        ...
        // Resolve linked places
        const linkedPlaces = day.placeIds
            .map(pid => (typeof places !== 'undefined' ? places.find(p => p.id === pid) : null))
            .filter(Boolean);
        
        // Resolve GPX place
        const gpxPlace = day.gpxPlaceId && typeof places !== 'undefined'
...
// ============= Day Edit Modal =============
function openDayEditModal(itineraryId, dateStr) {
    const itin = getItineraryById(itineraryId);
    if (!itin) return;
...
function renderDayPlacesSelector(selectedIds) {
    const list = document.getElementById('day-edit-places-list');
    if (!list) return;

    const allPlaces = typeof places !== 'undefined' ? places : [];
...
function renderDayGpxSelector(selectedGpxId) {
...
function closeCreateItineraryModal() {
    const modal = document.getElementById('create-itin-modal-overlay');
    if (modal) modal.classList.remove('active');
}

function submitCreateItinerary() {
    ...
    if (!name) {
        if (typeof showToast === 'function') showToast('נא להזין שם לטיול', 'error');
        return;
    }
    if (!start || !end) {
        if (typeof showToast === 'function') showToast('נא לבחור תאריכי התחלה וסיום', 'error');
        return;
    }
    if (new Date(start) > new Date(end)) {
        if (typeof showToast === 'function') showToast('תאריך ההתחלה חייב להיות לפני תאריך הסיום', 'error');
        return;
    }
    ...
    if (daysDiff > 60) {
        if (typeof showToast === 'function') showToast('לוח זמנים מוגבל ל-60 ימים מקסימום', 'error');
        return;
    }
    ...
    if (typeof showToast === 'function') showToast(`הלוח "${name}" נוצר בהצלחה!`, 'success');
}
...
function initItinerary() {
    ...
    // Desktop header itinerary toggling and close button inside panel
    ...
    // Auto-open panel on desktop if there is an active itinerary saved
    ...
    // Toggle quick add section in day edit modal
...
function renderCalendarView(itineraryId, customYear, customMonth) {
    const itin = getItineraryById(itineraryId);
    const ganttContainer = document.getElementById('gantt-container');
    if (!ganttContainer || !itin) return;
    ...
    if (customYear !== undefined && customMonth !== undefined) {
        calendarCurrentYear = customYear;
        calendarCurrentMonth = customMonth;
    } else if (calendarCurrentYear === null || calendarCurrentMonth === null) {
        const start = new Date(itin.startDate + 'T00:00:00');
        calendarCurrentYear = start.getFullYear();
        calendarCurrentMonth = start.getMonth();
    }
    ...
    // offset to align Sunday to first column (day of week: 0 for Sunday)
    const startOffset = firstDayOfMonth.getDay();
    const totalDaysInMonth = lastDayOfMonth.getDate();

    // Months names in Hebrew
        ...
        if (isItinDay) {
            ...
            cellContent += `
                ...
                <div class="cal-itin-icons">
                    ${itinDay.gpxPlaceId ? (() => {
                        const gpxP = typeof places !== 'undefined' ? places.find(p => p.id === itinDay.gpxPlaceId) : null;
                        if (!gpxP) return '';
    ...
    // Bind Calendar navigation month
    ...
    // Bind clicking on active itinerary day cells -> edit day modal
    ...
    // Bind map focus chips and place links
    bindGanttTimelineEvents(itineraryId);
}

// ============= Helper Functions for Binding Events =============
function bindGanttHeaderEvents(itineraryId) {
    // Bind back button
...
function bindGanttTimelineEvents(itineraryId) {
    const ganttContainer = document.getElementById('gantt-container');
    ...
    // Bind GPX track clicks, place chips, and calendar links to show/highlight on map
...
async function loadJewishHolidaysForYear(year) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (data.events) {
                ...
                // Rerender if we are currently viewing this year
                if (activeItineraryId) {
                    renderGanttView(activeItineraryId);
                }
            }
...
async function enrichItineraryWithHolidays(itin) {
    try {
...
function updateItineraryDates(itineraryId, newStartDate, newEndDate) {
    const itin = itineraries.find(it => it.id === itineraryId);
    if (!itin) return;
    ...
    // 3. Rebuild the days array from newStartDate to newEndDate
    const newDays = [];
    for (let d = new Date(newStart); d <= newEnd; d.setDate(d.getDate() + 1)) {
        ...
        if (shiftedDaysMap.has(dateStr)) {
            ...
            oldDayData.date = dateStr; // update date key
            newDays.push(oldDayData);
        } else {
            // Create a new blank day
            newDays.push({
                date: dateStr,
                title: '',
                notes: '',
    ...
    saveItineraries();
    syncItineraryToFirebase(itin);
    enrichItineraryWithHolidays(itin); // refresh holidays for new date range
    
    // Reset calendar monthly view pointers if needed
...
function openEditItineraryDatesModal(itineraryId) {
    const itin = getItineraryById(itineraryId);
    if (!itin) return;
    ...
    // Store context on modal dataset
    modal.dataset.itinId = itineraryId;

    // Add change listeners to check warning
    const checkWarning = () => {
...
function closeEditItineraryDatesModal() {
    const modal = document.getElementById('edit-itin-dates-modal-overlay');
    if (modal) modal.classList.remove('active');
}

function submitEditItineraryDates() {
    const modal = document.getElementById('edit-itin-dates-modal-overlay');
    if (!modal) return;
    ...
    if (!newStart || !newEnd) {
        if (typeof showToast === 'function') showToast('נא להזין תאריכי התחלה וסיום', 'error');
        return;
    }
    ...
    if (typeof showToast === 'function') {
        showToast('תאריכי הטיול עודכנו בהצלחה!', 'success');
    }
}

// ============= iCalendar (.ics) Export =============
function exportItineraryToICS(itin) {
    let icsContent = [
        'BEGIN:VCALENDAR',
        'VERSION:2.0',
    ...
    itin.days.forEach((day, idx) => {
        ...
        if (day.placeIds && day.placeIds.length > 0 && typeof places !== 'undefined') {
...
// Toggles display of itinerary panel in desktop view
...
async function handleQuickGmapsLinkImport() {
...
function reverseGeocodeQuickCoords(lat, lng, fallbackName) {
...
function submitQuickAddPlace() {
    ...
    if (typeof savePlaces === 'function') savePlaces();
    ...
    if (placesListContainer) {
        ...
        // Render again with the new place in the list and selected!
        renderDayPlacesSelector(selectedIds);
    }

    // Hide quick add section and reset
    const sec = document.getElementById('quick-add-place-section');
...
function updateGpxRangeUI(placeId, currentStart, currentEnd) {
    ...
    if (place && place.gpxData && place.gpxData.length > 0) {
        ...
        // Find other days in this trip sharing the same GPX
        let otherDaysHtml = '';
...
function shiftItineraryDays(itineraryId, fromDateStr, shiftDays) {
    const itin = getItineraryById(itineraryId);
    if (!itin || !fromDateStr || shiftDays <= 0) return;

    // Convert dates and shift all days from fromDateStr onwards
    itin.days.forEach(day => {
    ...
    // Save
    saveItineraries();
    if (typeof syncItineraryToFirebase === 'function') {
        syncItineraryToFirebase(itin);
    }
...
function swapItineraryDays(itineraryId, date1, date2) {
    const itin = getItineraryById(itineraryId);
    ...
    // Swap content properties (keep date properties unchanged)
    const temp = {
        title: day1.title,
        notes: day1.notes,
...
// Auto-initialize when DOM is ready and app.js has loaded
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => setTimeout(initItinerary, 100));
} else {
